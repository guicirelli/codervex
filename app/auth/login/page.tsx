...
export default function LoginPage() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage = err.errors?.[0]?.message || 'Error signing in. Please check your credentials.'
  ...
  const handleSocialLogin = (provider: 'oauth_google' | 'oauth_github') => {
    try {
      ...
      if (!signIn) {
        console.error('signIn is null or undefined')
        toast.error('Authentication service is not available. Please refresh the page.')
        return
      }

      console.log('signIn object:', signIn)
      console.log('signIn.authenticateWithRedirect type:', typeof signIn.authenticateWithRedirect)
      ...
      signIn.authenticateWithRedirect({
        ...
        if (errorMsg.toLowerCase().includes('not found') || 
          ...
          errorMsg = `No account found with this ${provider === 'oauth_google' ? 'Google' : 'GitHub'} account. Please sign up first.`
        }
